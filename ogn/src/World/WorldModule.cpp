...
{
	LOG_DEBUG(LogSystem::csl_color_blue, "player [%s] enter world", player->getName().c_str());
	return true;
}

bool WorldModule::onLeaveWorld(Player* player, Dictionary& dict)
{
	LOG_DEBUG(LogSystem::csl_color_blue, "player [%s] leave world", player->getName().c_str());
	return true;
}
...
{
	addEntity(plr);
	auto itr = mMapPlayer.find(plr->getAccountId());
	if (itr != mMapPlayer.end())
		return NULL;

	mMapPlayer.insert(std::make_pair(plr->getAccountId(), plr));
	return plr;
}
...
void WorldModule::removePlayer(Player* plr)
{
	...
	mMapPlayer.erase(plr->getAccountId());
